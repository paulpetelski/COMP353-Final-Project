...
eSSN = Employee.query.with_entities(Employee.ssn).distinct()
employeeChoices = [(row[0],row[0]) for row in eSSN]

pName = Project.query.with_entities(Project.pnumber).distinct()
projectChoices = [(row[0],row[0]) for row in pName]



class AssignForm(FlaskForm):

    ssn = SelectField('Employee', choices=employeeChoices)
    pnumber = SelectField('Project', choices=projectChoices)
    hours = IntegerField('Hours', validators=[DataRequired()])
    submit = SubmitField('Add Assignment')


    def validate_pnumber(self, pnumber):    
        assign = Works_On.query.filter_by(essn=self.ssn.data, pno=self.pnumber.data).first()
        if assign:
            raise ValidationError('This employee is already assigned to this project. Please choose a different project.')
